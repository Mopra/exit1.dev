...
| `--background` / `--sidebar` | `#15151B` ≈ `oklch(0.20 0.014 285)` | Canvas + sidebar share the baseline so the sidebar reads as part of the page chrome, not an elevated panel |
| `--card` / `--secondary` | `oklch(0.235 0.014 285)` | Elevated panels (+0.035) |
...
| `--input` | `oklch(0.3250 0 0)` | Form-control borders (slightly lifted from `--border` so inputs read as interactive) |

Use the theme's `--shadow-sm/md/lg/xl/2xl` tokens to model elevation — never invent custom shadows. Decorative glows are forbidden (see §4a).
...
- **HTTP timing stage labels** (`--stage-dns`, `-connect`, `-tls`, `-ttfb`): warm-leaning categorical chips used on Logs page request-timing labels and legends.
- **Phase chart fills** (`--phase-dns`, `-connect`, `-tls`, `-ttfb`): cool blue ramp (lightest → darkest = smallest → largest band) used by the stacked PhaseStackChart fills. Distinct from `--stage-*` because chart fills need a calmer, monochromatic ramp than label chips. Dark-mode only.
- **Pixel-card variants** (`--pixel-{default|blue|yellow|pink}-{1|2|3}`): three-stop fills for marketing/empty-state pixel art.
- **Aurora** (`--aurora-{1..4}`, `-glow`, `-ring-{outer|inner}`): premium glow card hues. Currently disabled by flat-mode (§4a).
- **`--favicon-offline`**: red dot composited over the public status page favicon when monitored services are down. Read by `PublicStatus.tsx` as inline SVG.
- **`--receipt-accent`**: hex-only token (PDF generators don't accept oklch) used by `lib/pdf-receipt.ts`. Override via `config/receipt.json` for per-deployment branding rather than editing `style.css`.
...
3. One primary CTA button (muted teal-green `--primary` / `#3F9081`).
