...
/**
 * Send a one-time "alerts temporarily paused" email when the circuit breaker trips.
 * Tells the user their monitors are still running but notifications are paused
 * because an abnormal volume was detected.
 */
export const sendAlertsPausedEmail = async (
  userId: string,
  tier: 'free' | 'nano' | 'scale',
  cooldownMs: number
): Promise<void> => {
  try {
    ...
    try {
      ...
      const err = clerkError as { status?: number; errors?: Array<{ code?: string }> };
      if (err?.status === 404 || err?.errors?.[0]?.code === 'resource_not_found') return;
      throw clerkError;
    }
    const userEmail = user.primaryEmailAddress?.emailAddress || user.emailAddresses[0]?.emailAddress;
    if (!userEmail) return;
    ...
    await resend.emails.send({
      from: fromAddress,
      to: userEmail,
      subject: 'Notifications temporarily paused — unusual alert volume detected',
      html: `
        <div style="font-family:Inter,ui-sans-serif,system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif;line-height:1.6;padding:16px;background:#0b1220;color:#e2e8f0">
          <div style="max-width:560px;margin:0 auto;background:rgba(2,6,23,0.6);backdrop-filter:blur(12px);border:1px solid rgba(148,163,184,0.15);border-radius:12px;padding:20px">
            <h2 style="margin:0 0 8px 0">Notifications Temporarily Paused</h2>
            <p style="margin:0 0 12px 0;color:#94a3b8">${formatDateForCheck(new Date())}</p>

            <div style="margin:12px 0;padding:12px;border-radius:8px;background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.2)">
              <p style="margin:0;color:#e2e8f0">We detected an unusual volume of alerts for your account and have <strong>temporarily paused all notifications</strong> to prevent spam.</p>
            </div>
            ...
            <div style="margin:16px 0;padding:12px;border-radius:8px;background:rgba(148,163,184,0.06);border:1px solid rgba(148,163,184,0.1)">
              ...
              <ul style="margin:0;padding-left:20px;color:#94a3b8">
                <li>Your monitors are still running and tracking uptime as normal</li>
                <li>All notifications (email, webhooks, SMS) are paused for approximately ${cooldownMinutes} minutes</li>
                <li>Notifications will automatically resume after the pause period</li>
                <li>This is usually caused by temporary network issues that trigger multiple false alerts</li>
              </ul>
            </div>

            <p style="margin:16px 0 0 0;color:#94a3b8;font-size:14px">If you continue to receive this notification frequently, please check your monitors for flapping checks (sites that rapidly toggle between up and down).</p>
          </div>
        </div>
      `,
    });

    logger.info(`Sent circuit-breaker notification to ${userEmail} for user ${userId}`);
  } catch (error) {
    logger.error(`Failed to send circuit-breaker notification for user ${userId}:`, error);
  }
};
